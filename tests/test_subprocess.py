...
class call_Test(TestCase):
    def test_cmd_is_str(self):
        ...
        cmd = 'ls'
        with self.assertRaises(pxul.subprocess.ArgumentsError):
            pxul.subprocess.call(cmd)

    def test_returns_triple(self):
        "Should return a triple (out, err, retcode)"
        cmd = 'ls'
        out, err, ret = pxul.subprocess.call([cmd],
                                             stdout=pxul.subprocess.PIPE,
                                             stderr=pxul.subprocess.PIPE)
    ...
    def test_returns_namedtuple(self):
        ...
        result = pxul.subprocess.call(['echo', 'hello', 'world'],
                                      stdout=pxul.subprocess.PIPE,
                                      stderr=pxul.subprocess.PIPE)
        self.assertIsInstance(result, pxul.subprocess.Result)
...
class run_Test(TestCase):
    def test_default_ok(self):
        ...
        res = pxul.subprocess.run(['echo', 'hello'])
    ...
    def test_raises_false_bad_command(self):
        ...
        res = pxul.subprocess.run(['touch', '/unallowed'],
                                  capture='both', raises=False)
...
class Command_Test(TestCase):
    ...
    def test_call_no_args(self):
        ...
        echo = pxul.subprocess.Builder(['echo'], capture='both')
        out, err, ret = echo()
    ...
    def test_object_immutable(self):
        ...
        echo = pxul.subprocess.Builder(['echo'], capture='silent')
